...
| 2  | 搭建前后端并实现接口 | 1. API 接口实现: 提供 /api/predict (或类似) 接口用于接收前端 POST 请求；2. 请求处理: 正确解析前端发送的 JSON 数据（如 {"text": "You are stupid"}）；3. 响应格式: 返回统一格式的 JSON 响应，包含预测结果；4. 跨域支持: 配置了 CORS，允许前端域名访问后端 API。 | \[√] 符合 ｜前端验证: 打开前端页面，输入文本并提交，观察是否能正确显示预测结果。｜
